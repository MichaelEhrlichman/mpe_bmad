...
real, parameter :: mu0 = 0.02
real, parameter :: wi0 = 0.001
...
real, parameter :: meas_x2 = 2.0

integer :: ndata
real, allocatable :: data_x(:), data_y(:)
...
subroutine read_data()
  open(10,name='along_0p-thin.dat')
  ndata = 0
  do
    read(10,*,end=10)
    ndata = ndata + 1
  ...
  allocate(data_x(ndata))
  allocate(data_y(ndata))

  open(10,name='along_0p-thin.dat')
  do i=1,ndata
    read(10,*) data_x(i), data_y(i)
  enddo
  close(10)
end subroutine
...
subroutine take_meas()
  ...
  do i=1,nmeas,2
    call box_muller(n1,n2)
    meas(i) = a*exp(-b*t(i)) + sig*n1
    meas(i+1) = a*exp(-b*t(i+1)) + sig*n2
  enddo
end subroutine
...
subroutine likelihood()
  lik = exp(-0.5*sum((spread(meas,2,nmodel)-outer_product(t,va))**2,1)/(sig**2))
  lik = lik/sum(lik)
end subroutine
