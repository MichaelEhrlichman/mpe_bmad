program moga_ma
  ...
  !for magnet errors
  integer, parameter :: MAX_ERR_SETS=5
  ...
  character(10) error_mask(MAX_ERR_SETS)
    ...
    do while(.true.)
      ...
      if( sane_chrom ) then
        ...
        if( feasible .and. (generate_feasible_seeds_only .lt. 0) ) then
                  ...
                  if(str_match_wild(ring%ele(j)%name,error_mask(i))) then
                    ...
                    if( attribute_ix .gt. 0 ) then
                      ...
                      if(trim(error_property(i)) == 'K1') then
                        write(set_str,'(a,es15.6)') 'K1 = ', (1.0+magnet_error)*value_of_attribute(ring%ele(j),'K1')
                      elseif(trim(error_property(i)) == 'A1') then  !skew gradient errors
                        write(set_str,'(a,es15.6)') 'A1 = ', magnet_error*value_of_attribute(ring%ele(j),'K1')*value_of_attribute(ring%ele(j),'L')
                      endif
                      call set_ele_attribute(ring%ele(j), set_str, err_flag)
                    else
          ...
          ring6 = ring
          call set_on_off(rfcavity$, ring6, on$)
          bmad_com%radiation_damping_on = .true.
          call clear_lat_1turn_mats(ring6)
          co(0)%vec = 0.0d0
          call twiss_and_track(ring6,co,status)
          do j=1,n_ma_loc
            call momentum_aperture_one(ring6,co,n_ma_turn,6,max_deltam(j),.true.)
          enddo
          bmad_com%radiation_damping_on = .false.
          objs(2) = sum(0.1d0 - max_deltam(:)%pos) / 0.1d0 / n_ma_loc
          objs(3) = sum(0.1d0 - abs(max_deltam(:)%neg)) / 0.1d0 / n_ma_loc
        endif
      endif
    enddo
  endif
