program aperture_and_lifetime
  ...
  elseif( trim(stepping) == 'range_ix' ) then
    ...
    max_deltam(1:n_ma_locs)%s=max(max_deltam(1:n_ma_locs)%s,0.00001)
  ...
  if( aperture_file == 'generate_new' ) then
    if(master) then
      ...
      do while(.true.)
        ...
        if(ix .lt. 0) then
          write(*,'(a,i6,a)') "Worker ", myrank, " received exit signal." 
          exit
        endif
        call momentum_aperture_one(ring,co,n_turn,6,max_deltam(ix),.true.)
  ...
  write(*,'(a,i6,a)') "Worker ", myrank, " calling mpi_finalize." 
  call mpi_finalize(mpierr)
  write(*,'(a,i6,a)') "Worker ", myrank, " end." 
