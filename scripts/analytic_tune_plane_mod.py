...
def analyticTunePlane(fig,ax,Qxmin,Qxmax,Qymin,Qymax,periodicity=1):
	...
	nrange = [x*periodicity for x in range(0,nMax+1)]
	for n in nrange:
		for p in range(-(pMax),(pMax+1)):
			...
			for q in range(-qMax,qMax+1):
				my_gcd = reduce(gcd,(p,q,n))
				if(q==0):
					pass;
				elif(not visible_test(p,q,n,Qxmin,Qxmax,Qymin,Qymax)):
					pass;
				elif((abs(p)+abs(q)) > pqMax):
					pass;
				elif( (my_gcd != 1) and ( abs(p)==abs(q) or p==0 or q==0) ):
						pass;
				elif( (my_gcd != 1) and ((n/my_gcd)%periodicity == 0) ):
						pass;
				else:
					x1 = Qxmin
					x2 = Qxmax
