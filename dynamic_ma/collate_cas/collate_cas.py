...
amp_indexes = []
amp_factors = []
with open('amp_factors.dat','r') as f:
	for l in f:
...
if rolls:
	roll_amp_indexes = []
	roll_amp_factors = []
	with open('amp_factors_roll.dat','r') as f:
		for l in f:
			if not l.strip().startswith("#"):
				s = [x.strip() for x in l.split()]
				roll_amp_indexes.append(s[0:1][0])
				roll_amp_factors.append(s[2:3][0])  #y only ... x not impacted by rolls
...
if rolls:
	for ix,fname,col in roll_keys:
		if fname != '*':
			CASdata = np.loadtxt(fname,usecols=(0,int(col)))
			if first:
				CAS2 = np.zeros_like(CASdata)
				CAS2[:,0] = CASdata[:,0]
				first = False
			roll_amp_factor = float(roll_amp_factors[roll_amp_indexes.index(ix)])
			CAS2[:,1] = CAS2[:,1] + np.square(roll_amp_factor*CASdata[:,1])

with open('accumulated_CAS_at_extraction.dat','w') as f:
	for freq,x in CAS2:
...
with open('PSD_at_extraction.dat','w') as f:
	for freq,x in PSD:
		f.write("{:10.4f}   {:13.5e}\n".format(freq,x))

with open('filtered_PSD_at_extraction.dat','w') as f:
...
with open('filtered_CAS_at_extraction.dat','w') as f:
