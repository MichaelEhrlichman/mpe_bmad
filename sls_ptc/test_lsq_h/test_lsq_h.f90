...
tx_limiter = -1.0  !normalize strength adjustment so that no magnet changes by more than this relative amount
include_oct = .false.

           !terms_struct(stage, name, target, weight)
termdefs(1)  = terms_struct(1, 0, '21000', 0.0, 0.1)         ! Qx
termdefs(2)  = terms_struct(1, 0, '30000', 0.0, 0.1)         ! 3Qx
termdefs(3)  = terms_struct(1, 0, '10110', 0.0, 0.1)         ! Qx
termdefs(4)  = terms_struct(1, 0, '10020', 0.0, 0.1)         ! Qx-2Qy
termdefs(5)  = terms_struct(1, 0, '10200', 0.0, 0.1)         ! Qx+2Qy
termdefs(6)  = terms_struct(1, 0, '20001', 0.0, 0.1)         ! Synchro-betatron resonances ???
termdefs(7)  = terms_struct(1, 0, '00201', 0.0, 0.1)         ! Momentum-dependence of beta functions ???
termdefs(8)  = terms_struct(1, 0, '10002', 0.0, 0.1)         ! 2nd order dispersion
...
termdefs(20)  = terms_struct(1, 1, '00001',  1.0, 1.)     ! chrom x
termdefs(21)  = terms_struct(2, 1, '00002',  0.0, 1.0)     ! 2nd order chrom x
termdefs(22)  = terms_struct(3, 1, '00003',  0.0, 1.0)     ! 3rd order chrom x
termdefs(23)  = terms_struct(1, 2, '00001',  1.0, 1.)     ! chrom y
termdefs(24)  = terms_struct(2, 2, '00002',  0.0, 1.0)     ! 2nd order chrom y
termdefs(25)  = terms_struct(3, 2, '00003',  0.0, 1.0)     ! 3rd order chrom y
termdefs(26)  = terms_struct(6, 6, '00001', -0.1, 1.0)     ! dz/dp: first order momentum compaction (related by negative sign)
termdefs(27)  = terms_struct(6, 6, '00002',  0.0, 1.0)     ! d2x/dp2: second order momentum compaction (related by negative sign)
termdefs(28)  = terms_struct(6, 6, '00003', -1.0, 1.0)     ! d3x/dp3: third order momentum compaction (related by negative sign)
termdefs(29)  = terms_struct(6, 6, '00004', -5.0, 1.0)     ! d3x/dp3: third order momentum compaction (related by negative sign)

do i=1,max_terms
  if(termdefs(i)%stage .lt. 1) exit
enddo
n_terms = i-1
n_stages = maxval(termdefs(1:n_terms)%stage)
...
do while( loop_stages )
  ...
  h=cgetpb(F,S2=-1)   ! cgetpb includes 1/(-2*i_) 
    ...
    if(my_worker_num == 1) then
      ...
      write(*,'(A14,A25,A25,A10)') "Term", "Value", "Merit", "Stage"
      do i=1,n_terms
        write(*,'(A,I3,A,A,F25.10,F25.10,I10)') "     ", termdefs(i)%F_index, " ", termdefs(i)%name,  terms(i), terms(i)*termdefs(i)%weight, termdefs(i)%stage
    ...
    if(my_worker_num == 1) then
      ...
      if(iterations .lt. 1) then
        ...
        do while(menu)
          ...
          write(6,*) "  1) Iterate LSQ at current stage"
          write(6,*) "  2) Move on to next stage"
          write(6,*) "  3) Write Jacobian to file"
          write(6,*) "  4) Terminate program"
          write(6,*) "  5) Change LSQ limiter (currently ", tx_limiter, ")"
          write(6,*) "  6) Manual adjustment"
          ...
          elseif( selection == 5 ) then
            write(6,'(A)',advance='no') "Enter new limiter value: "
            read(5,*) tx_limiter
            menu = .true.
          elseif( selection == 6 ) then
            write(6,'(A)',advance='no') "Enter magnet family name to adjust: "
            read(5,*) manual_mag
            write(6,'(A)',advance='no') "Enter amount by which to adjust the family: "
            read(5,*) manual_tx
            p=>r%start
            do i=1,r%n
              if(trim(p%mag%name) == trim(manual_mag)) then
                call add(p,3,1,manual_tx)  !FOO sextupoles only
                if(my_worker_num == 1) then
                  write(*,'(A,I3,A,A,A,ES17.7,A,ES17.7,A)') "Adjusted family ", j, "   ", p%mag%name, " by ", manual_mag, " (new strength: ", p%mag%bn(3), " )"  !FOO sextupoles only
                endif
                exit
              endif
              p=>p%next
