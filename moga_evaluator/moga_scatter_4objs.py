...
axmax = 1.01*np.amax([datax])
axmin = 0.99*np.amin([datax])
aymax = 1.01*np.amax([datay])
aymin = 0.99*np.amin([datay])
awmax = np.amax([dataw])
awmin = np.amin([dataw])
...
# Plot the data
# indexes = [1,2,3,4]
# datablock = zip(dataseed,dataw,datax,datay,dataz)
# for datapoint in datablock:
# 	ax.plot(indexes,datapoint[1:5],picker=4,label=datapoint[0])
datablock = zip(dataseed,dataw,datax,datay,dataz)
for dp in datablock:
	sc=ax.scatter(dp[2],dp[3],c=dp[1],cmap=pyplot.cm.jet,s=2.0,picker=3,label=dp[0],vmin=awmin,vmax=awmax)
cbar = pyplot.colorbar(sc)
cbar.ax.set_ylabel('On-Energy DA')

ax.set_xlabel('Positive MA')
ax.set_ylabel('Negative MA')
